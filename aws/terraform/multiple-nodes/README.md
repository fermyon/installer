...
All Hashistack (Nomad, Consul), Traefik and Fermyon platform processes can run
on single or multiple EC2 instances. The data is stored on AWS EBS.

Since the infrastructure uses a single node with elastic ip to receive requests,
it might have a single point of failure.
